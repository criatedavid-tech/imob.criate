...
import { OFFICIAL_WHATSAPP_PAI_PHONE } from "./whatsappPaiIdentity";
...
async function getPlatformPaiPhone(): Promise<string> {
  ...
  if (error) {
    ...
    if (/phone_normalized/i.test(error.message || "")) return OFFICIAL_WHATSAPP_PAI_PHONE;
    throw error;
  }
  const value = normalizePhoneBR(optionalString(data?.phone_normalized)) || OFFICIAL_WHATSAPP_PAI_PHONE;
