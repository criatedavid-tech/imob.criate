...
## Atualização 2026-07-17 — Exclusão real em Conversas, Locação, Leads e Equipe

Usuário reportou, com prints, que Conversas e Locação (e por extensão Leads e
Equipe) não tinham NENHUMA forma de apagar registro — só editar/mudar status.
Em Locação isso era visível de forma concreta: contratos já `encerrado`
ficavam presos pra sempre, porque o único botão de ação ("Encerrar
contrato") só aparecia pra contratos `ativo`. Pedido do usuário: "isso deve
ser possível" nas quatro áreas, incluindo revogar convite pendente em
Equipe.

Implementado (sessão local, antes de puxar as 6 commits que o Codex tinha
publicado em paralelo — reconciliado depois via merge, ver adiante):

- **Locação** — `DELETE /api/locacao/contracts/:id` em
  `server/routes/locacao.ts`: apaga o contrato e, em cascata manual, as
  cobranças (`imf_rental_payments`) geradas pra ele. Botão de lixeira em
  `LocacaoArea.tsx`, ao lado do lápis de editar, visível em qualquer status.
- **Leads** — `DELETE /api/leads/:id` em `server/routes/leads.ts`: mesmo
  padrão de escopo das outras rotas do arquivo (via `property_id` do broker
  + `isBrokerOwner` pra membro só apagar os leads que possui). Botão de
  lixeira no card do lead em `NegociosArea.tsx`.
- **Equipe** — convite pendente (gerado, ainda não aceito) não tinha como
  ser cancelado antes disso, só deixar expirar (48h) ou remover membro
  *depois* de aceito. Adicionado `GET /api/equipe/invites` (lista convites
  ...
  (só o titular) em `server/routes/equipe.ts`. Nova seção "Convites
  pendentes" em `EquipeArea.tsx`.
- **Conversas** — implementado originalmente como
  `DELETE /api/conversas/:customerPhone` (modelo antigo, uma linha por
  telefone).

Testado ao vivo contra a API em produção (`imobiflow-v2.fly.dev`) com uma
conta descartável: contrato+cobrança, conversa+mensagem+tag+nota, lead e
convite criados (direto no banco via service role ou pela própria API),
os 4 DELETEs chamados de verdade, confirmado que cada um sumiu do banco,
conta de teste removida no final. `npx tsc --noEmit` e `npm run build`
limpos; deploy Fly v89 saudável.

### Reconciliação com o UUID nativo por ticket (commit `d50e938`, local)

Ao tentar `git push origin v2`, o push foi rejeitado — o Codex tinha
publicado 6 commits em paralelo nesta mesma janela, incluindo exatamente o
UUID nativo por ticket de conversa (ver seção anterior) e a desativação das
operações financeiras de cliente por padrão. `git merge origin/v2` gerou
conflito em dois arquivos: `DOCUMENTACAO.md` (esperado — os dois lados
editaram a cauda do arquivo) e `src/experience/ConversasArea.tsx` (o cabeçalho
da conversa selecionada, onde o Codex passou a mostrar telefone + código do
ticket ao lado do toggle de IA, e eu tinha inserido o botão de apagar no
mesmo bloco).

O que **não** gerou conflito de texto mas exigia correção manual: minha rota
`DELETE /api/conversas/:customerPhone` continuava operando no modelo antigo
(telefone como chave, `imf_conversation_tag_links`/`imf_conversation_notes`
por `customer_phone`) depois que o merge trouxe o refactor de tickets — e
essas duas tabelas, na migration nova, passaram a exigir `ticket_id NOT
NULL` (o `customer_phone` continua existindo só em `imf_conversation_messages`,
não nas outras duas). Ou seja: um merge textualmente limpo que teria
deployado uma rota quebrada (chamando `canAccessConversation`, função que o
Codex removeu/renomeou para `canAccessTicket`, e filtrando tabelas por uma
coluna que não é mais a chave certa). Corrigido reescrevendo a rota para
`DELETE /api/conversas/:ticketId`, usando `getConversationTicket` +
`canAccessTicket` (mesmo padrão de todas as outras rotas de ticket no
arquivo) e apagando por `ticket_id` nas quatro tabelas antes de apagar o
próprio ticket. Semântica: apaga UM ciclo/ticket específico, não o
telefone inteiro — coerente com o resto da tela, que já lista tickets
antigos e ativos como linhas separadas.

Confirmado antes do merge que a migration `20260717_conversation_ticket_cycles.sql`
já estava aplicada no Supabase real (consulta direta via `service_role`
retornou a tabela `imf_conversation_tickets` e a coluna
`followup_conversations.ticket_id`), então não havia risco de branch nova
rodar contra schema velho.

O conflito em `DOCUMENTACAO.md` foi resolvido descartando o bloco de
changelog antigo que eu tinha acrescentado (a Asaas-key, o overflow do
Financeiro e a limpeza de dado morto já estavam documentados de forma mais
atual nas seções 1-17 da reorganização que o Codex publicou nesse mesmo
lote — a mesma reorganização que eu tinha pedido num prompt anterior desta
